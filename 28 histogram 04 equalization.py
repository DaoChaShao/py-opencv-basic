...
# -*- Coding: UTF-8 -*-
# @Time     :   2025/4/26 17:14
# @Author   :   Shawn
# @Version  :   Version 0.1.0
# @File     :   28 histogram 04 equalization.py
# @Desc     :   
...
def main() -> None:
    ...
    if image is not None:
        ...
        equalized = equalizeHist(image)
        print(f"The type of image is {type(equalized)}")
        print(f"The shape of image is {equalized.shape}")
        print(f"The dtype of image is {equalized.dtype}")
        print(f"The pixel size of image is {equalized.size}")
        imshow(WINDOW_EQU, equalized)

        # Set the histogram
        histogram = calcHist([equalized], [0], None, [256], [0, 256]).flatten()
