...
def main() -> None:
    ...
    WINDOW_DILATE_WITHOUT_KERNEL: str = "Dilate without kernel"
    WINDOW_DILATE_WITH_KERNEL: str = "Dilate with kernel"
    ...
    if image is not None:
        ...
        image_without_kernel = dilate(image, None)
        ...
        imshow(WINDOW_DILATE_WITHOUT_KERNEL, image_without_kernel)

        # Transform the image with kernel
        kernel = ones((5, 5), dtype=image.dtype)
        image_with_kernel = dilate(image, kernel)
        ...
        imshow(WINDOW_DILATE_WITH_KERNEL, image_with_kernel)
